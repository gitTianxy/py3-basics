...
class ThreadLocalDemo:
    ...
    ---
    ThreadLocal的作用:
        用定义全局变量/共享变量的形式定义了'在线程间相互独立的同类资源', 简化了线程参数传递的方式.
    用法:
...
class ThreadDemo:
    ...
    def __init__(self):
        ...
        # init
        threads = []
        t1 = threading.Thread(target=ThreadDemo.listen_music, args=('kevin-music', 2,))
        threads.append(t1)
        t2 = threading.Thread(target=ThreadDemo.watch_movie, args=('kevin-movie', 5,))
        threads.append(t2)
        # start threads
        for t in threads:
            t.start()
...
class MyThreadDemo:
    """
    self-defined thread: a runnable class
    """
    global mutex
    ...
    def __init__(self):
        ...
        threads.append(t1)
        t2 = self.MyThread("watch movie", 3)
        threads.append(t2)
        # start threads
        for t in threads:
            t.start()
